...
export default function SchedulePage() {
  ...
  const signUpExclusive = async (activityId: string, allSlotActivityIds: string[]) => {
    if (!user) return;
    if (inFlight.has(activityId)) return;

    // Check if we are already signed up for this specific activity
    const isAlreadySignedUp = (signups[activityId] ?? []).includes(user.uid);
    if (isAlreadySignedUp) return;

    setInFlight(prev => {
      const next = new Set(prev);
      allSlotActivityIds.forEach(id => next.add(id));
      return next;
    });
    ...
    setSignups(prev => {
      const next = { ...prev };
      allSlotActivityIds.forEach(id => {
        if (id === activityId) {
          next[id] = [...(prev[id] ?? []).filter(uid => uid !== user.uid), user.uid];
        } else {
          next[id] = (prev[id] ?? []).filter(uid => uid !== user.uid);
        }
      });
      return next;
    });
    ...
    try {
      ...
      allSlotActivityIds.forEach(id => {
        if (id === activityId) {
          updates[`signups.${id}`] = arrayUnion(user.uid);
        } else {
          updates[`signups.${id}`] = arrayRemove(user.uid);
        }
      });

      await setDoc(doc(db, "schedules", today), updates, { merge: true });
      
      clearTimeout(timeoutId);
      showToast("הרישום עודכן בהצלחה!", "success");
    } catch (e) {
      clearTimeout(timeoutId);
      console.error(e);
      // Rollback
      ...
      setInFlight(prev => {
        const next = new Set(prev);
        allSlotActivityIds.forEach(id => next.delete(id));
        return next;
      });
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="space-y-6">
        {timeSlots.length > 0 ? timeSlots.map((slot, idx) => {
          ...
          if (isAlternativeSlot) {
            ...
            return (
              <motion.div
                ...
                className={`bg-[var(--surface)] border-2 rounded-[2rem] p-8 shadow-sm transition-all flex flex-col gap-6 ${
                  chosenItem ? 'border-teal-500/30' : 'border-amber-500/20 bg-amber-500/[0.01]'
                }`}
              >
                {/* Time slot header */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-[var(--border)] pb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-teal-500/10 text-teal-600 rounded-2xl flex flex-col items-center justify-center font-black text-xs shrink-0">
                      <span>{slot.startTime}</span>
                      <span className="text-[9px] opacity-60">{slot.endTime}</span>
                    </div>
                    <div>
                      <h4 className="text-lg font-black text-[var(--foreground)]">חלון זמן — בחירת פעילות</h4>
                      <p className="text-xs text-[var(--muted)]">בחלון זמן זה קיימות מספר פעילויות במקביל. אנא בחר את הפעילות הרצויה:</p>
                    </div>
                  </div>
                  {!chosenItem && (
                    <span className="flex items-center gap-1 text-[10px] font-black text-amber-600 bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-lg self-start md:self-center">
                      ...
                      >
                        ...
                        {!isPast && (
                          <div className="pt-2 mt-auto">
                            {inFlight.has(a.id) ? (
                              <div className="flex justify-center py-2"><Loader2 className="w-4 h-4 animate-spin text-teal-500" /></div>
                            ) : isSelected ? (
                              <span className="text-xs font-black text-rose-500 flex items-center gap-1 hover:underline">
                                <X className="w-3.5 h-3.5" /> ביטול הבחירה
                              </span>
                            ) : (
                              <span className="text-xs font-black text-teal-600 flex items-center gap-1 group-hover:translate-x-[-2px] transition-transform">
                                <Check className="w-3.5 h-3.5" /> בחר בפעילות זו
                              </span>
                            )}
                          </div>
                        )}
            ...
            return (
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h4 className="text-xl font-black mb-1 flex items-center gap-2 text-[var(--foreground)]">
