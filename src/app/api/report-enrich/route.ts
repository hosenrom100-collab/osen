...
const PROFILE_LABELS: Record<string, Record<string, string>> = {
  emotional: {
    anxious: "חרדתי ומתוח", depressed: "דיכאוני ומסתגר",
    unstable: "תנודתי וסוער", stable: "יציב ומאוזן יחסית",
  },
  family: {
    supported: "עטוף ותומך מאוד", complex: "תמיכה משפחתית מתוחה או מורכבת",
    isolated: "בדידות / חוסר תמיכה משפחתית",
  },
  personality: {
  ...
  attendance: {
    ...
    unstable: "נוכחות תנודתית עם נסיונות גיוס עצמי",
  },
  futureDirection: {
  ...
  processStage: {
    ...
    transitioning: "לקראת מעבר או סיום",
  },
};
...
function describeProfile(profile: ParticipantProfile): string {
  const lines: string[] = [];
  for (const key of ["emotional", "family", "personality", "regulation", "social", "trust", "attendance", "futureDirection", "processStage"] as const) {
...
אם סופקו "הערות נוספות מהצוות המטפל" - שלב אותן בתוך הטקסט בצורה טבעית ואינטגרטיבית (לא כפסקה נפרדת ולא כציטוט), כך שהן משפיעות על הניסוח וההדגשים בכל הקטעים הרלוונטיים, בהתאם למאפיינים שנבחרו בשאלון.
