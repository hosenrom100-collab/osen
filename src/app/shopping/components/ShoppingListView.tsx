...
export function ShoppingListView({
  ...
  return (
    <div dir="rtl" className="w-full max-w-2xl mx-auto pb-24 px-2.5 sm:px-4">
      {/* ── Framework chips: every option visible at once, no menu to open. Sticky, so the
           current scope stays in view while scrolling a long list. Height is fixed (52px)
           because the category headers below stick just underneath it. ── */}
      <div className="sticky top-0 z-20 -mx-2.5 sm:-mx-4 px-2.5 sm:px-4 bg-[var(--background)]/90 backdrop-blur-md">
        <div role="tablist" aria-label="סינון לפי מסגרת" className="flex items-center gap-2 py-2 overflow-x-auto no-scrollbar">
                ...
                className={`h-9 px-3.5 rounded-full text-[13px] font-semibold whitespace-nowrap shrink-0 flex items-center gap-1.5 border transition-colors cursor-pointer ${
                  active
                    ? chip.id === "all"
                      ...
                      : `${fw?.activeBg ?? "bg-[var(--accent)]"} !text-white border-transparent`
                    : "bg-[var(--surface)] text-[var(--foreground)]/80 border-[var(--border)] hover:bg-[var(--foreground)]/[0.05]"
                }`}
              >
                {fw && !active && <span aria-hidden className={`w-2 h-2 rounded-full ${fw.dot}`} />}
                <span>{chip.label}</span>
                <span className={`text-xs font-bold tabular-nums ${active ? "opacity-80" : "text-[var(--muted)]"}`}>
                  {chip.count}
                </span>
              </button>
            );
      ...
      {(totalItems > 0 || activeCategory !== null) && (
        <div className="pt-3 pb-4" aria-live="polite">
          <div className="flex items-center gap-2">
            <div className="flex items-baseline gap-2 min-w-0 flex-1">
              <span className="text-2xl font-extrabold tabular-nums text-[var(--foreground)] leading-none">
            ...
            {urgentCount > 0 && (
              <button
                ...
                className={`h-9 px-3 rounded-full text-[13px] font-semibold border flex items-center gap-1.5 whitespace-nowrap shrink-0 cursor-pointer transition-colors ${
                  urgentFilterActive
              ...
              >
                ...
                <span>דחוף</span>
                <span className="text-xs font-bold tabular-nums">{urgentCount}</span>
              </button>
            )}
            ...
            {activeCategory !== null && (
              <button
                ...
                className="h-9 px-3 rounded-full text-[13px] font-semibold bg-[var(--accent-soft)] border border-[var(--accent-line)] text-[var(--accent-text)] flex items-center gap-1.5 whitespace-nowrap shrink-0 cursor-pointer"
            ...
            {canPurchase && totalItems > 0 && (
              <button
                ...
                className="h-9 w-9 sm:w-auto sm:px-3 rounded-full text-[13px] font-semibold border bg-[var(--surface)] text-[var(--foreground)] border-[var(--border)] hover:bg-[var(--foreground)]/[0.05] flex items-center justify-center gap-1.5 shrink-0 cursor-pointer transition-colors"
              >
      ...
      {allDone ? (
        <div className="py-12 px-6 text-center bg-emerald-500/[0.06] border border-emerald-500/20 rounded-2xl my-2">
          <div className="text-4xl mb-2">🎉</div>
          <h3 className="text-lg font-extrabold text-emerald-700 dark:text-emerald-300">סיימנו! הכל נרכש</h3>
        ...
        <div className={`${cardClass} py-14 px-6 text-center my-2`}>
          ...
          <h3 className="text-lg font-extrabold text-[var(--foreground)]">הרשימה ריקה</h3>
        ...
        categories.map((cat) => {
          ...
          return (
            <section key={cat} className={`${cardClass} mb-4`}>
              ...
              <header className="sticky top-[3.25rem] z-10 flex items-center gap-2.5 px-4 py-2.5 bg-[var(--surface)]/95 backdrop-blur border-b border-[var(--border)]">
                ...
                <div className={`${cardClass} mt-3 divide-y divide-[var(--border)]`}>
                  {sessionPurchased.map((item) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-3 min-h-[52px] hover:bg-[var(--foreground)]/[0.03] transition-colors text-right cursor-pointer border-none bg-transparent"
      ...
      <AnimatePresence>
        {undo && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            role="status"
            className="fixed bottom-44 md:bottom-28 left-1/2 -translate-x-1/2 z-[90] bg-zinc-900 !text-white rounded-2xl shadow-[var(--shadow-pop)] pl-3 pr-4 py-2 flex items-center gap-3 max-w-[92vw]"
          >
            ...
            <button
              ...
              className="h-9 px-3 rounded-lg text-sm font-bold text-indigo-300 hover:bg-white/10 flex items-center gap-1.5 shrink-0 cursor-pointer border-none bg-transparent"
            >
