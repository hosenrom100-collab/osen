...
export function ParticipantSurveyStep({
  ...
  return (
    <div className="flex-1 overflow-y-auto space-y-6 px-1 py-2 scrollbar-thin">
      <div className="bg-violet-500 text-white rounded-3xl p-6 shadow-md mb-2">
      ...
      {onEnrich && onNotesChange && (
        <div className="bg-violet-50/60 border border-violet-200 rounded-2xl p-4 space-y-2">
          <label className="text-[11px] font-bold text-violet-800 flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5" />
            הערות נוספות להעשרה חכמה (אופציונלי)
          </label>
          <p className="text-[11px] text-violet-500 font-bold leading-relaxed">
      ...
      <div className="pt-4 flex justify-between border-t border-slate-100 shrink-0">
        <button
          onClick={onCancel}
          className="bg-slate-50 hover:bg-slate-100 border border-slate-200 px-6 py-3 rounded-2xl text-[11px] font-bold uppercase tracking-wider transition-all cursor-pointer border-none"
        >
          ביטול
        </button>
        <div className="flex items-center gap-2">
          {onEnrich && (
            <button
              ...
              className="bg-white hover:bg-violet-50 disabled:opacity-50 text-violet-700 font-bold px-5 py-3 rounded-2xl text-[11px] uppercase tracking-wider transition-all border border-violet-200 cursor-pointer flex items-center gap-1.5"
            >
          ...
          <button
            onClick={onSubmit}
            disabled={!isProfileComplete(profile) || enriching}
            className="bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white font-bold px-8 py-3 rounded-2xl text-[11px] uppercase tracking-wider transition-all shadow-lg cursor-pointer border-none flex items-center gap-1.5"
          >
