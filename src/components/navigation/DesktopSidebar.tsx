    ...
    <aside className="hidden md:flex w-60 shrink-0 h-screen sticky top-0 flex-col bg-sidebar-bg border-l border-border z-20 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-6 h-16 shrink-0 bg-background/50">
        ...
        <div className="flex flex-col">
          <span className="text-[15px] font-bold text-white tracking-tight leading-none">Hosen Connect</span>
              ...
              return (
                <Link key={href} href={href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 group ${
                    active
                      ? "bg-emerald-500/10 text-emerald-400 font-semibold shadow-[inset_0_0_0_1px_rgba(16,185,129,0.1)]"
                      : "text-slate-400 hover:text-slate-200 hover:bg-white/[0.03]"
                  }`}>
                  <Icon className={`w-4 h-4 shrink-0 transition-colors ${active ? "text-emerald-400" : "text-slate-500 group-hover:text-slate-300"}`} />
                  <span>{label}</span>
              ...
              {ADMIN_NAV.map(({ href, icon: Icon, label }) => {
                ...
                return (
                  <Link key={href} href={href}
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 group ${
                      active
                        ? "bg-slate-500/10 text-white font-semibold"
                        : "text-slate-400 hover:text-slate-200 hover:bg-white/[0.03]"
                    }`}>
                    <Icon className={`w-4 h-4 shrink-0 transition-colors ${active ? "text-white" : "text-slate-500 group-hover:text-slate-300"}`} />
                    <span>{label}</span>
                  </Link>
                );
              })}
      ...
      <div className="mt-auto border-t border-border p-4 bg-background/30">
        <Link href="/profile"
          ...
          <div className="relative">
            <div className="w-9 h-9 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center text-xs font-bold text-slate-300 transition-colors group-hover:border-emerald-500/50">
              {initials}
            </div>
